...
bool Config::LoadFromDisk() noexcept {
	...
	if (!file.is_open()) {
		GlobalLogger().m_warn << "**** Error opening properties file (load). Attempting to create new file...\n";
		return false;
	}
	...
	while (std::getline(file, line)) {
		...
		if (delimiterPos == std::string::npos) {
			GlobalLogger().m_error << "**** Invalid line in properties file: " << line << "\n";
			continue;
		}
...
bool Config::SaveToDisk() const noexcept {
	...
	if (!file.is_open()) {
		GlobalLogger().m_error << "**** Error opening properties file (save). \n";
		return false;
	}
	...
	try {
		...
		GlobalLogger().m_error << "**** Error while writing properties file: " << e.what() << "\n";
		return false;
	}

	GlobalLogger().m_info << "Properties file saved successfully.\n";
	file.close();
	return true;
}
