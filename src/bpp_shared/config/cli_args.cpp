...
Command ParseCommand(const std::string& s) {
	...
	if (s == "--pregen")
		return Command::PREGEN_CHUNKS;
	if (s == "--force-nether-spawn")
		return Command::FORCE_NETHER_SPAWN;
	if (s == "--max-players")
		return Command::MAX_PLAYERS;
	if (s == "--view-distance")
		return Command::VIEW_DISTANCE;
	if (s == "--level-seed")
		return Command::LEVEL_SEED;
	if (s == "--seed")
		return Command::LEVEL_SEED;
	if (s == "--whitelist")
		return Command::ENABLE_WHITELIST;
	if (s == "--disable-nether")
		return Command::DISABLE_NETHER;
