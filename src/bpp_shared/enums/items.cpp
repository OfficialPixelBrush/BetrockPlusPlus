...
bool IsSword(ItemId id) {
	return (id == ITEM_SWORD_IRON || id == ITEM_SWORD_WOOD || id == ITEM_SWORD_STONE || id == ITEM_SWORD_DIAMOND ||
	        id == ITEM_SWORD_GOLD);
}

bool IsPickaxe(ItemId id) {
	return (id == ITEM_PICKAXE_IRON || id == ITEM_PICKAXE_WOOD || id == ITEM_PICKAXE_STONE ||
	        id == ITEM_PICKAXE_DIAMOND || id == ITEM_PICKAXE_GOLD);
}

bool IsAxe(ItemId id) {
	return (id == ITEM_AXE_IRON || id == ITEM_AXE_WOOD || id == ITEM_AXE_STONE || id == ITEM_AXE_DIAMOND ||
	        id == ITEM_AXE_GOLD);
}

bool IsShovel(ItemId id) {
	return (id == ITEM_SHOVEL_IRON || id == ITEM_SHOVEL_WOOD || id == ITEM_SHOVEL_STONE || id == ITEM_SHOVEL_DIAMOND ||
	        id == ITEM_SHOVEL_GOLD);
}
...
bool IsTool(ItemId id) {
	return IsShovel(id) || IsAxe(id) || IsPickaxe(id) || IsHoe(id) || id == ITEM_FLINT_AND_STEEL ||
	       id == ITEM_FISHING_ROD || id == ITEM_SHEARS;
}
...
ItemDamage GetMaxDurability(ItemId id) {
	...
	case ITEM_HELMET_LEATHER:
		return DURABILITY_HELMET_LEATHER;
	case ITEM_CHESTPLATE_LEATHER:
		return DURABILITY_CHEST_LEATHER;
	case ITEM_LEGGINGS_LEATHER:
		return DURABILITY_LEGS_LEATHER;
	case ITEM_BOOTS_LEATHER:
		return DURABILITY_BOOTS_LEATHER;

		// Armor - Chainmail
	case ITEM_HELMET_CHAINMAIL:
		return DURABILITY_HELMET_CHAINMAIL;
	case ITEM_CHESTPLATE_CHAINMAIL:
		return DURABILITY_CHEST_CHAINMAIL;
	case ITEM_LEGGINGS_CHAINMAIL:
		return DURABILITY_LEGS_CHAINMAIL;
	case ITEM_BOOTS_CHAINMAIL:
		return DURABILITY_BOOTS_CHAINMAIL;

		// Armor - Iron
	case ITEM_HELMET_IRON:
		return DURABILITY_HELMET_IRON;
	case ITEM_CHESTPLATE_IRON:
		return DURABILITY_CHEST_IRON;
	case ITEM_LEGGINGS_IRON:
		return DURABILITY_LEGS_IRON;
	case ITEM_BOOTS_IRON:
		return DURABILITY_BOOTS_IRON;

		// Armor - Diamond
	case ITEM_HELMET_DIAMOND:
		return DURABILITY_HELMET_DIAMOND;
	case ITEM_CHESTPLATE_DIAMOND:
		return DURABILITY_CHEST_DIAMOND;
	case ITEM_LEGGINGS_DIAMOND:
		return DURABILITY_LEGS_DIAMOND;
	case ITEM_BOOTS_DIAMOND:
		return DURABILITY_BOOTS_DIAMOND;

		// Armor - Gold
	case ITEM_HELMET_GOLD:
		return DURABILITY_HELMET_GOLD;
	case ITEM_CHESTPLATE_GOLD:
		return DURABILITY_CHEST_GOLD;
	case ITEM_LEGGINGS_GOLD:
		return DURABILITY_LEGS_GOLD;
	case ITEM_BOOTS_GOLD:
		return DURABILITY_BOOTS_GOLD;

		// Misc damageable
	case ITEM_FLINT_AND_STEEL:
		return DURABILITY_FLINT_AND_STEEL;
