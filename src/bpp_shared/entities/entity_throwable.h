...
struct ThrowableEntity : public Entity {
	// Default
	...
	// Spawn from position
	ThrowableEntity(Vec3 _position) : Entity() {
		this->InitThrowable();
		this->Teleport(_position);
	}

	// Spawn from entity
	ThrowableEntity(std::shared_ptr<MobileEntity> _owner) : Entity() {
		this->InitThrowable();
		if (!_owner) {
			GlobalLogger().error << "Spawned throwable with invalid entity pointer!\n";
			return;
		}
		this->owner = _owner;
		...
		this->SetHeading(heading, /*strength=*/1.5f, /*inaccuracy=*/1.0f);
	}

	void SetHeading(Vec3 _direction, float _strength, float _inaccuracy) {
...
private:
	...
	bool throwableInGround : 1 = false;
