...
public:
	...
	void GenerateOctaves(std::vector<double>& _noiseField, Vec3 _coordinate, Int32_3 _size, Vec3 _p_scale);
	// func_4103_a
...
inline NoiseOctavesPerlin::NoiseOctavesPerlin(Java::Random& _rand, int32_t _poctaves) : octaves(_poctaves) {
	for (size_t i = 0; i < size_t(octaves); ++i)
		generator_collection.push_back(NoisePerlin(_rand));
}
...
inline double NoiseOctavesPerlin::GenerateOctaves(Vec2 _offset) {
	...
	double scale = 1.0;
	for (size_t i = 0; i < size_t(octaves); ++i) {
		value += generator_collection[i].GenerateNoise(_offset * scale) / scale;
		scale /= 2.0;
	}
	return value;
}
...
inline void NoiseOctavesPerlin::GenerateOctaves(std::vector<double>& _noiseField, Vec3 _coordinate, Int32_3 _size,
	...
	for (size_t octave = 0; octave < size_t(octaves); ++octave) {
		generator_collection[octave].GenerateNoise(_noiseField, _coordinate, _size, _p_scale * multiplier, multiplier);
		multiplier /= 2.0;
	}
}
