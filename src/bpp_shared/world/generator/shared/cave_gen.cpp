...
const std::vector<CaveGenerator::CaveTunnel>& CaveGenerator::GetTunnelsForOrigin(Int2 _chunkOffset) {
	auto it = originCache.find(_chunkOffset);
	if (it != originCache.end())
		return it->second;

	if (originCache.size() >= M_ORIGIN_CACHE_LIMIT) {
		Int2 oldest = originCacheOrder.front();
		originCacheOrder.pop_front();
		originCache.erase(oldest);
	}

	auto emplaced = originCache.emplace(_chunkOffset, RecordOrigin(_chunkOffset));
	originCacheOrder.push_back(_chunkOffset);
...
void CaveGenerator::GenerateCavesForChunk(Chunk& _chunk, int64_t _seed) {
	...
	for (int32_t cXoffset = _chunk.cpos.x - carveExtent; cXoffset <= _chunk.cpos.x + carveExtent; ++cXoffset) {
		for (int32_t cZoffset = _chunk.cpos.z - carveExtent; cZoffset <= _chunk.cpos.z + carveExtent; ++cZoffset) {
			...
			for (const CaveTunnel& tunnel : GetTunnelsForOrigin({ cXoffset, cZoffset }))
				ApplyTunnel(_chunk, tunnel);
		}
	}
}

std::vector<CaveGenerator::CaveTunnel> CaveGenerator::RecordOrigin(Int2 _chunkOffset) {
	...
	for (int32_t caveIndex = 0; caveIndex < numberOfCaves; ++caveIndex) {
		...
		if (rand.NextInt(4) == 0) {
			tunnels.push_back(RecordTunnel(offset, 1.0f + rand.NextFloat() * 6.0f, 0.0f, 0.0f, -1, -1, 0.5));
		...
		for (int32_t nodeIndex = 0; nodeIndex < numberOfNodes; ++nodeIndex) {
			...
			tunnels.push_back(RecordTunnel(offset, isNetherCave ? tunnelRadius * 2.0f : tunnelRadius, carveYaw,
			                               carvePitch, 0, 0, 1.0));
		}
	}
...
CaveGenerator::CaveTunnel CaveGenerator::RecordTunnel(Vec3 _offset, float _tunnelRadius, float _carveYaw,
                                                      float _carvePitch, int32_t _tunnelStep, int32_t _tunnelLength,
	...
	tunnel.startStep = _tunnelStep;
	tunnel.tunnelLength = _tunnelLength;
	tunnel.tunnelRadius = _tunnelRadius;
	tunnel.isShaft = isShaft;
	...
	for (; _tunnelStep < _tunnelLength; ++_tunnelStep) {
		...
		if (!isShaft && _tunnelStep == branchPoint && _tunnelRadius > 1.0f) {
			tunnel.branches.push_back(RecordTunnel(_offset, rand2.NextFloat() * 0.5f + 0.5f,
			                                       _carveYaw - JavaMath::PI_FLOAT * 0.5f, _carvePitch / 3.0f,
			                                       _tunnelStep, _tunnelLength, 1.0));
			tunnel.branches.push_back(RecordTunnel(_offset, rand2.NextFloat() * 0.5f + 0.5f,
			                                       _carveYaw + JavaMath::PI_FLOAT * 0.5f, _carvePitch / 3.0f,
			                                       _tunnelStep, _tunnelLength, 1.0));
			return tunnel;
		...
		bool eligible = isShaft || (rand2.NextInt(4) != 0);
		tunnel.steps.push_back({ _offset, radiusXz, radiusY, eligible });
	}

	return tunnel;
}

void CaveGenerator::ApplyTunnel(Chunk& _chunk, const CaveTunnel& _tunnel) {
	...
	for (size_t i = 0; i < _tunnel.steps.size(); ++i) {
		const CaveTunnelStep& step = _tunnel.steps[i];
		if (!step.eligible)
			continue;

		int32_t tunnelStep = _tunnel.startStep + int32_t(i);
		double dx = step.offset.x - chunkCenterX;
		double dz = step.offset.z - chunkCenterZ;
		double dist = double(_tunnel.tunnelLength - tunnelStep);
		double limit = double(_tunnel.tunnelRadius + 2.0f + 16.0f);
		if ((dx * dx + dz * dz - dist * dist) > (limit * limit))
			return;

		if (step.offset.x >= chunkCenterX - 16.0 - step.radiusXz * 2.0 &&
		    step.offset.z >= chunkCenterZ - 16.0 - step.radiusXz * 2.0 &&
		    step.offset.x <= chunkCenterX + 16.0 + step.radiusXz * 2.0 &&
		    step.offset.z <= chunkCenterZ + 16.0 + step.radiusXz * 2.0) {
			int32_t xMin = MathHelper::FloorDouble(step.offset.x - step.radiusXz) - _chunk.cpos.x * 16 - 1;
			int32_t xMax = MathHelper::FloorDouble(step.offset.x + step.radiusXz) - _chunk.cpos.x * 16 + 1;
			int32_t yMin = MathHelper::FloorDouble(step.offset.y - step.radiusY) - 1;
			int32_t yMax = MathHelper::FloorDouble(step.offset.y + step.radiusY) + 1;
			int32_t zMin = MathHelper::FloorDouble(step.offset.z - step.radiusXz) - _chunk.cpos.z * 16 - 1;
			int32_t zMax = MathHelper::FloorDouble(step.offset.z + step.radiusXz) - _chunk.cpos.z * 16 + 1;
				...
				for (int32_t blockZ = zMin; !fluidIsPresent && blockZ < zMax; ++blockZ) {
					for (int32_t blockY = yMax + 1; !fluidIsPresent && blockY >= yMin - 1; --blockY) {
						if (blockY >= 0 && blockY < CHUNK_HEIGHT) {
							...
							if (!isNetherCave && (blockType == BLOCK_WATER_FLOWING || blockType == BLOCK_WATER_STILL))
								fluidIsPresent = true;
							// Nether caver check
			...
			if (!fluidIsPresent) {
				for (int32_t blockX = xMin; blockX < xMax; ++blockX) {
					double centerDx = (double(blockX + _chunk.cpos.x * 16) + 0.5 - step.offset.x) / step.radiusXz;

					for (int32_t blockZ = zMin; blockZ < zMax; ++blockZ) {
						double centerDz = (double(blockZ + _chunk.cpos.z * 16) + 0.5 - step.offset.z) / step.radiusXz;
						...
						if (centerDx * centerDx + centerDz * centerDz < 1.0) {
							...
							for (int32_t blockY = yMax - 1; blockY >= yMin; --blockY) {
								...
								double centerDy = (double(blockY) + 0.5 - step.offset.y) / step.radiusY;
								if (centerDy > -0.7 &&
	...
	for (const CaveTunnel& branch : _tunnel.branches)
		ApplyTunnel(_chunk, branch);
}
