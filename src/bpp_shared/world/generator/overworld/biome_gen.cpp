...
void BiomeGenerator::GenerateBiomeMap(Biome biomeMap[], std::vector<double>& temperature, std::vector<double>& humidity,
	...
	this->temperatureNoiseGen.GenerateOctaves(temperature, blockPos, max_area, Vec2{ double(0.025f), double(0.025f) },
	                                            0.25);
	this->humidityNoiseGen.GenerateOctaves(humidity, blockPos, max_area, Vec2{ double(0.05f), double(0.05f) },
	                                         1.0 / 3.0);
	this->weirdnessNoiseGen.GenerateOctaves(weirdness, blockPos, max_area, Vec2{ 0.25, 0.25 }, 0.5882352941176471);
	size_t index = 0;
...
Biome BiomeGenerator::GetBiomeAtPoint(Int2 worldPos) {
	...
	this->temperatureNoiseGen.GenerateOctaves(temp, Int2{ worldPos.x, worldPos.y }, Int32_2{ 1, 1 },
	                                            Vec2{ double(0.025f), double(0.025f) }, 0.25);
	this->humidityNoiseGen.GenerateOctaves(humi, Int2{ worldPos.x, worldPos.y }, Int32_2{ 1, 1 },
	                                         Vec2{ double(0.05f), double(0.05f) }, 1.0 / 3.0);
	this->weirdnessNoiseGen.GenerateOctaves(weird, Int2{ worldPos.x, worldPos.y }, Int32_2{ 1, 1 },
...
void BiomeGenerator::GenerateTemperature(std::vector<double>& temperature, std::vector<double>& weirdness,
	...
	this->temperatureNoiseGen.GenerateOctaves(temperature, blockPos, max, Vec2{ double(0.025f), double(0.025f) },
	                                            0.25);
