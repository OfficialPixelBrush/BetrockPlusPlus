...
namespace Blocks {
    ...
    struct StairsBuilder {
        ...
        StairsBuilder facing(StairsDirection direction) const { StairsBuilder b = *this; b.m_direction = direction; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_direction) }; }
        operator Block() const { return asBlock(); }
    ...
    struct FluidBuilder {
        ...
        FluidBuilder level(uint8_t level) const { FluidBuilder b = *this; b.m_level = std::min<uint8_t>(level, 7); return b; }
        FluidBuilder falling(bool falling) const { FluidBuilder b = *this; b.m_falling = falling; return b; }

        Block asBlock() const {
            uint8_t data = m_level & 0x7;
            if (m_falling) data |= 0x8;
    ...
    struct SaplingBuilder {
        SaplingBuilder type(WoodType type) const { SaplingBuilder b = *this; b.m_type = type; return b; }
        SaplingBuilder readyToGrow(bool readyToGrow) const { SaplingBuilder b = *this; b.m_readyToGrow = readyToGrow; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_type);
            if (m_readyToGrow) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct LogBuilder {
        LogBuilder type(WoodType type) const { LogBuilder b = *this; b.m_type = type; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_type) }; }
        operator Block() const { return asBlock(); }
    ...
    struct LeavesBuilder {
        LeavesBuilder type(WoodType type) const { LeavesBuilder b = *this; b.m_type = type; return b; }
        LeavesBuilder decaying(bool decaying) const { LeavesBuilder b = *this; b.m_decaying = decaying; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_type);
            if (m_decaying) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct TallGrassBuilder {
        TallGrassBuilder type(TallGrassType type) const { TallGrassBuilder b = *this; b.m_type = type; return b; }
    ...
    struct WoolBuilder {
        WoolBuilder color(WoolColor color) const { WoolBuilder b = *this; b.m_color = color; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_color) }; }
        operator Block() const { return asBlock(); }
    ...
    struct SlabBuilder {
        ...
        SlabBuilder type(SlabType type) const { SlabBuilder b = *this; b.m_type = type; return b; }
    ...
    struct TorchBuilder {
        ...
        TorchBuilder attachment(TorchAttachment attachment) const { TorchBuilder b = *this; b.m_attachment = attachment; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_attachment) }; }
        operator Block() const { return asBlock(); }
    ...
    struct WallFacingBuilder {
        ...
        WallFacingBuilder facing(WallFacing facing) const { WallFacingBuilder b = *this; b.m_facing = facing; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_facing) }; }
        operator Block() const { return asBlock(); }
    ...
    struct BedBuilder {
        BedBuilder facing(Direction direction) const { BedBuilder b = *this; b.m_direction = direction; return b; }
        BedBuilder occupied(bool occupied) const { BedBuilder b = *this; b.m_occupied = occupied; return b; }
        BedBuilder head(bool head) const { BedBuilder b = *this; b.m_head = head; return b; }

        Block asBlock() const {
            uint8_t data;
            switch (m_direction) {
            case Direction::South: data = 0; break;
            case Direction::West:  data = 1; break;
            case Direction::North: data = 2; break;
            case Direction::East:  data = 3; break;
            }
            if (m_occupied) data |= 0x4;
            if (m_head) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct PoweredRailBuilder {
        ...
        PoweredRailBuilder shape(RailShape shape) const { PoweredRailBuilder b = *this; b.m_shape = shape; return b; }
        PoweredRailBuilder powered(bool powered) const { PoweredRailBuilder b = *this; b.m_powered = powered; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_shape) & 0x7;
            if (m_powered) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct PistonBuilder {
        ...
        PistonBuilder facing(PistonFacing facing) const { PistonBuilder b = *this; b.m_facing = facing; return b; }
        PistonBuilder extended(bool extended) const { PistonBuilder b = *this; b.m_extended = extended; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_facing);
            if (m_extended) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct PistonHeadBuilder {
        PistonHeadBuilder facing(PistonFacing facing) const { PistonHeadBuilder b = *this; b.m_facing = facing; return b; }
        PistonHeadBuilder sticky(bool sticky) const { PistonHeadBuilder b = *this; b.m_sticky = sticky; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_facing);
            if (m_sticky) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct DoorBuilder {
        ...
        DoorBuilder rotation(uint8_t rotation) const { DoorBuilder b = *this; b.m_rotation = rotation & 0x3; return b; }
        DoorBuilder open(bool open) const { DoorBuilder b = *this; b.m_open = open; return b; }
        DoorBuilder upperHalf(bool upperHalf) const { DoorBuilder b = *this; b.m_upperHalf = upperHalf; return b; }

        Block asBlock() const {
            uint8_t data = m_rotation & 0x3;
            if (m_open) data |= 0x4;
            if (m_upperHalf) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct SignBuilder {
        SignBuilder rotation(uint8_t rotation) const { SignBuilder b = *this; b.m_rotation = rotation & 0xF; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_rotation & 0xF) }; }
        operator Block() const { return asBlock(); }
    ...
    struct RailBuilder {
        RailBuilder shape(RailShape shape) const { RailBuilder b = *this; b.m_shape = shape; return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_shape) }; }
        operator Block() const { return asBlock(); }
    ...
    struct LeverBuilder {
        LeverBuilder mount(LeverMount mount) const { LeverBuilder b = *this; b.m_mount = mount; return b; }
        LeverBuilder on(bool on) const { LeverBuilder b = *this; b.m_on = on; return b; }
    ...
    struct ButtonBuilder {
        ButtonBuilder mount(ButtonMount mount) const { ButtonBuilder b = *this; b.m_mount = mount; return b; }
        ButtonBuilder pressed(bool pressed) const { ButtonBuilder b = *this; b.m_pressed = pressed; return b; }

        Block asBlock() const {
            uint8_t data = static_cast<uint8_t>(m_mount);
            if (m_pressed) data |= 0x8;
            return Block{ m_id, data };
        }
        operator Block() const { return asBlock(); }
    ...
    struct SnowLayerBuilder {
        SnowLayerBuilder height(uint8_t height) const { SnowLayerBuilder b = *this; b.m_height = std::min<uint8_t>(height, 7); return b; }

        Block asBlock() const { return Block{ m_id, static_cast<uint8_t>(m_height & 0x7) }; }
        operator Block() const { return asBlock(); }
    ...
    struct TrapdoorBuilder {
        TrapdoorBuilder facing(Direction direction) const { TrapdoorBuilder b = *this; b.m_direction = direction; return b; }
        TrapdoorBuilder open(bool open) const { TrapdoorBuilder b = *this; b.m_open = open; return b; }

        Block asBlock() const {
            uint8_t data;
            switch (m_direction) {
            case Direction::South: data = 0; break;
            case Direction::North: data = 1; break;
            case Direction::East:  data = 2; break;
            case Direction::West:  data = 3; break;
            }
            if (m_open) data |= 0x4;
            return Block{ m_id, data };
        }
    ...
    struct PumpkinBuilder {
        ...
        PumpkinBuilder facing(Direction direction) const { PumpkinBuilder b = *this; b.m_direction = direction; return b; }
    ...
    struct RepeaterBuilder {
        ...
        RepeaterBuilder facing(Direction direction) const { RepeaterBuilder b = *this; b.m_direction = direction; return b; }
        RepeaterBuilder delay(uint8_t delay) const { RepeaterBuilder b = *this; b.m_delay = std::min<uint8_t>(delay, 3); return b; }

        int delayTicks() const { return (static_cast<int>(m_delay) + 1) * 2; }

        Block asBlock() const {
            uint8_t data;
            switch (m_direction) {
            case Direction::North: data = 0; break;
            case Direction::East:  data = 1; break;
