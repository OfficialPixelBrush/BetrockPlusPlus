...
bool PacketDispatcher::dispatch(PacketId packetId, PlayerSession& session, WorldManager& sessionWorld, Server& server) {
	...
	case PacketId::KeepAlive: {
		...
		pkt.Deserialize(session.m_stream);
	...
	case PacketId::PlaceBlock: {
		...
		pkt.Deserialize(session.m_stream);
		HandlePacket::PlaceBlock(pkt, session, sessionWorld, server.m_gameRuntime);
		break;
	}
	...
	case PacketId::Animation: {
		...
		HandlePacket::Animation(pkt, session,
		                        session.m_dimension == 0 ? server.m_overworldEntityTracker : server.m_hellEntityTracker);
		break;
	}
	...
	default:
		GlobalLogger().m_warn << "UNHANDLED packet 0x" << std::hex << static_cast<int>(packetId) << "\n";
