...
#if defined(__clang__) && defined(_LIBCPP_VERSION) && defined(BS_THREAD_POOL_MODULE) && (__cplusplus >= 202002L) &&    \
...
struct [[nodiscard]] version {
	constexpr version(const std::uint64_t _major, const std::uint64_t _minor, const std::uint64_t _patch) noexcept
...
struct [[nodiscard]] pr_task {
     ...
     */
	explicit pr_task(task_t&& _task,
	                 const priority_t _priority = 0) noexcept(std::is_nothrow_move_constructible_v<task_t>)
...
public:
     ...
     * @param nublocks_ The desired number of blocks to divide the range into.
     */
	blocks(const T _first_index, const T _index_after_last, const std::size_t _nublocks) noexcept
	    : nublocks(_nublocks), first_index(_first_index), index_after_last(_index_after_last) {
		if (index_after_last > first_index) {
			const std::size_t total_size = static_cast<std::size_t>(index_after_last - first_index);
			nublocks = std::min(nublocks, total_size);
			block_size = total_size / nublocks;
			remainder = total_size % nublocks;
			if (block_size == 0) {
				block_size = 1;
				nublocks = (total_size > 1) ? total_size : 1;
			}
		} else {
			nublocks = 0;
		}
	...
	[[nodiscard]] T end(const std::size_t _block) const noexcept {
		return (_block == nublocks - 1) ? index_after_last : start(_block + 1);
	}
     ...
     */
	[[nodiscard]] std::size_t get_nublocks() const noexcept {
		return nublocks;
	}
	...
	[[nodiscard]] T start(const std::size_t _block) const noexcept {
		return first_index + static_cast<T>(_block * block_size) +
		       static_cast<T>(_block < remainder ? _block : remainder);
	}
...
struct task_and_future {
	...
	explicit task_and_future(F&& _func) {
		...
		task = [task = std::forward<F>(_func), promise = std::move(promise)]() mutable {
#ifdef __cpp_exceptions
			try {
#endif
				if constexpr (std::is_void_v<R>) {
...
[[nodiscard]] inline std::optional<std::vector<bool>> get_os_process_affinity() {
	...
	DWORD_PTR systemask = 0;
	if (GetProcessAffinityMask(GetCurrentProcess(), &process_mask, &systemask) == 0)
		return std::nullopt;
#ifdef __cpp_lib_int_pow2
	const std::size_t nucpus = static_cast<std::size_t>(std::bit_width(systemask));
#else
	std::size_t nucpus = 0;
	if (systemask != 0) {
		nucpus = 1;
		while ((systemask >>= 1U) != 0U)
			++nucpus;
	}
#endif
	std::vector<bool> affinity(nucpus);
	for (std::size_t i = 0; i < nucpus; ++i)
	...
	if (sched_getaffinity(getpid(), sizeof(cpu_set_t), &cpu_set) != 0)
		return std::nullopt;
	const int nucpus = get_nprocs();
	if (nucpus < 1)
		return std::nullopt;
	std::vector<bool> affinity(static_cast<std::size_t>(nucpus));
...
struct common_index_type<T1, T2,
	...
	using type = std::conditional_t<
	    ...
	    std::conditional_t<
	        ...
	        std::conditional_t<larger_size == 2 || (sizeof(S) == 4 && sizeof(U) < 4), std::int32_t, std::int64_t>>,
...
public:
     ...
     *
     * @param nuthreads The number of threads to use.
     */
	explicit thread_pool(const std::size_t _nuthreads) : thread_pool(_nuthreads, [] {}) {}
     ...
     */
	template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
	thread_pool(const std::size_t _nuthreads, F&& _init) {
		create_threads(_nuthreads, std::forward<F>(_init));
	}
     ...
     * @param nublocks The maximum number of blocks to split the loop into. The default is 0, which means the number of blocks will be equal to the number of threads in the pool.
	...
	void detach_blocks(const T1 _first_index, const T2 _index_after_last, F&& _block, const std::size_t _nublocks = 0,
	                   const priority_t _priority = 0) {
		enqueue_blocks<T, F, void, false>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                                  std::forward<F>(_block), _nublocks, _priority);
	}
     ...
     */
	template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F>
	void detach_loop(const T1 _first_index, const T2 _index_after_last, F&& _loop, const std::size_t _nublocks = 0,
	                 const priority_t _priority = 0) {
		enqueue_loop<T, F, false>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                          std::forward<F>(_loop), _nublocks, _priority);
	}
     ...
     */
	template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F>
	void detach_sequence(const T1 _first_index, const T2 _index_after_last, F&& _sequence,
	                     const priority_t _priority = 0) {
		return enqueue_sequence<T, F, void, false>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                                           std::forward<F>(_sequence), _priority);
	}
     ...
     * @param nuthreads The number of threads to use.
     */
	void reset(const std::size_t _nuthreads) {
		reset(_nuthreads, [](std::size_t) {});
	}
     ...
     */
	template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
	void reset(const std::size_t _nuthreads, F&& _init) {
		if constexpr (pause_enabled) {
			std::unique_lock tasks_lock(tasks_mutex);
			const bool was_paused = paused;
			paused = true;
			tasks_lock.unlock();
			reset_pool(_nuthreads, std::forward<F>(_init));
			tasks_lock.lock();
			paused = was_paused;
			tasks_lock.unlock();
			if (!was_paused)
	...
	[[nodiscard]] multi_future<R> submit_blocks(const T1 _first_index, const T2 _index_after_last, F&& _block,
	                                            const std::size_t _nublocks = 0, const priority_t _priority = 0) {
		return enqueue_blocks<T, F, R, true>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                                     std::forward<F>(_block), _nublocks, _priority);
	}
	...
	[[nodiscard]] multi_future<R> submit_bulk(const I _first, const I _last, const priority_t _priority = 0) {
		if (_first != _last) {
			const std::size_t nutasks = static_cast<std::size_t>(std::distance(_first, _last));
			multi_future<R> all_futures;
			all_futures.reserve(nutasks);
			std::vector<task_t> all_tasks;
			all_tasks.reserve(nutasks);
			for (I it = _first; it != _last; ++it) {
     ...
     */
	template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F>
	[[nodiscard]] multi_future<void> submit_loop(const T1 _first_index, const T2 _index_after_last, F&& _loop,
	                                             const std::size_t _nublocks = 0, const priority_t _priority = 0) {
		return enqueue_loop<T, F, true>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                                std::forward<F>(_loop), _nublocks, _priority);
	}
	...
	[[nodiscard]] multi_future<R> submit_sequence(const T1 _first_index, const T2 _index_after_last, F&& _sequence,
	                                              const priority_t _priority = 0) {
		return enqueue_sequence<T, F, R, true>(static_cast<T>(_first_index), static_cast<T>(_index_after_last),
		                                       std::forward<F>(_sequence), _priority);
	}
	...
	[[nodiscard]] std::future<R> submit_task(F&& _task, const priority_t _priority = 0) {
		...
		detach_task(std::move(ft.task), _priority);
	...
	bool wait_for(const std::chrono::duration<R, P>& _duration) {
		...
		std::unique_lock tasks_lock(tasks_mutex);
		waiting = true;
		const bool status = tasks_done_cv.wait_for(tasks_lock, _duration, [this] {
			if constexpr (pause_enabled)
				return (tasks_running == 0) && (paused || tasks.empty());
			else
				return (tasks_running == 0) && tasks.empty();
	...
	bool wait_until(const std::chrono::time_point<C, D>& _timeout_time) {
		...
		std::unique_lock tasks_lock(tasks_mutex);
		waiting = true;
		const bool status = tasks_done_cv.wait_until(tasks_lock, _timeout_time, [this] {
			if constexpr (pause_enabled)
				return (tasks_running == 0) && (paused || tasks.empty());
			else
				return (tasks_running == 0) && tasks.empty();
...
private:
     ...
     */
	template <typename F>
	void create_threads(const std::size_t _nuthreads, F&& _init) {
		if constexpr (std::is_invocable_v<F, std::size_t>) {
		...
		thread_count = determine_thread_count(_nuthreads);
...
#ifdef __cpp_lib_jthread
			                      (const std::stop_token& _stop_token) { worker(_stop_token, i); }
#else
			                      { worker(i); }
#endif
			);
     ...
     * @param nublocks The maximum number of blocks to split the loop into.
	...
	[[nodiscard]] N enqueue_blocks(const T _first_index, const T _index_after_last, F&& _block, std::size_t _nublocks,
	                               const priority_t _priority = 0) {
		if (_index_after_last > _first_index) {
			using block_task_t = block_task<T, F, R>;
			const std::shared_ptr<std::decay_t<F>> block_ptr = std::make_shared<std::decay_t<F>>(
			    std::forward<F>(_block));
			const blocks blks(_first_index, _index_after_last, _nublocks ? _nublocks : thread_count);
			_nublocks = blks.get_nublocks();
			std::vector<std::conditional_t<submit, block_task_t, task_t>> all_tasks;
			all_tasks.reserve(_nublocks);
			for (std::size_t i = 0; i < _nublocks; ++i)
	...
	[[nodiscard]] N enqueue_loop(const T _first_index, const T _index_after_last, F&& _loop, std::size_t _nublocks,
	                             const priority_t _priority = 0) {
		if (_index_after_last > _first_index) {
	...
	template <typename T, typename F, typename R, bool submit,
	          typename N = std::conditional_t<submit, multi_future<R>, void>>
	[[nodiscard]] N enqueue_sequence(const T _first_index, const T _index_after_last, F&& _sequence,
	                                 const priority_t _priority = 0) {
		if (_index_after_last > _first_index) {
			using sequence_task_t = sequence_task<T, F, R>;
			const std::shared_ptr<std::decay_t<F>> sequence_ptr = std::make_shared<std::decay_t<F>>(
			    std::forward<F>(_sequence));
     ...
     */
	template <typename F>
	void reset_pool(const std::size_t _nuthreads, F&& _init) {
		wait();
#ifndef __cpp_lib_jthread
		destroy_threads();
#endif
...
public:
	...
	void print(const T&... _items) {
		const std::scoped_lock strealock(streamutex);
